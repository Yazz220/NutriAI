...
Folio is a book-first Expo app for personal digital cookbooks. Users create styled cookbooks and capture recipes from links, pasted text, screenshots, public supported social-video links, or direct video files. Folio extracts the structured recipe for reasoning, generates the complete designed recipe page, and places it directly into the appropriate book. The embedded AI chef answers questions from the active page and the user's wider recipe collection.
...
- **Book reader**: a swipeable reader with a physical cover, recipe pages, add-page controls, and the Folio assistant.
- **Recipe import**: one durable multimodal pipeline accepts URL, text, image, public YouTube, TikTok, Instagram, or Facebook video links, and direct supported video sources. Pinterest retains the guided file, screenshot, audio, or text fallback.
...
- **Folio assistant**: an assistant-ui powered chat with tool-calling that can scale servings, substitute ingredients, start timers, guide steps, and patch the recipe graph live.
...
- `extract-recipe`: provider-neutral recipe evidence and multimodal extraction for URL, text, image, resolved video sources, acquired social-video evidence, and transcripts from uploaded audio. The default model is Qwen3.6-35B-A3B via OpenRouter, with independent video and speech-to-text model overrides.
...
`recipe_graph` remains the canonical machine-readable recipe used by Folio. The selected generated page image is the user-facing book page. Saved recipe edits therefore generate a replacement page before the graph and selected version are switched together.
...
```text
Create a cookbook
  -> share a recipe to Folio or choose Add page
  -> Folio extracts the Recipe Graph
  -> the explicit, default, or sole cookbook supplies the visual identity
  -> Folio generates and publishes the complete page
  -> open the book, flip to the recipe, and Ask Folio while cooking
```
...
app/
  ...
  (book)/
    ...
    share.tsx                  native Share to Folio handoff
    [cookbookId]/
...
- The Folio assistant uses `@assistant-ui/react-native` with a `LocalRuntime`, device-persisted conversation history, and a bridge to the `nosh-chat` Edge Function; each active chat is scoped by the current page and book.
...
SUPADATA_API_KEY                           server-only social-video acquisition key
SUPADATA_API_BASE                          optional; defaults to https://api.supadata.ai/v1
SUPADATA_ENABLED_PLATFORMS                 optional comma list; defaults to youtube,tiktok,instagram,facebook
...
| preview / production | Folio | `com.yaz12.nosh` | `nosh` |
...
Private, pre-launch, and production-bound. The active surface is the cookbook shelf, book reader, direct recipe capture, settings, and contextual Folio assistant.
