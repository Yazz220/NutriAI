...
export function mapPage(
  ...
  if ((selectedVersion?.image_url || selectedVersion?.storage_path) && row.style_id && isCompletePage) {
    page.pageImage = {
  ...
  if ((selectedVersion?.image_url || selectedVersion?.storage_path) && row.style_id && !isCompletePage) {
    page.artAsset = {
      id: selectedVersion.id,
      pageId: row.id,
      artUrl: selectedVersion.image_url ?? undefined,
      storagePath: selectedVersion.storage_path ?? undefined,
      styleId: row.style_id as CookbookPageStyleId,
...
export async function createCookbook(input: CreateCookbookInput): Promise<Cookbook> {
  ...
  const { data, error } = await supabase
    ...
    .rpc('create_cookbook_for_current_user', {
      ...
      p_cover_title_color_id: normalizeCoverTitleColorId(input.coverTitleColorId),
      p_cover_title_placement_id: normalizeCoverTitlePlacementId(input.coverTitlePlacementId),
