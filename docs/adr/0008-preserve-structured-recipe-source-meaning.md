...
The URL adapter considers every Schema.org Recipe node. It selects an explicit `mainEntity`, a URL-matched candidate, or one candidate that is substantially more complete. It does not auto-publish when multiple candidates remain equally plausible. This keeps the deterministic fast path, but removes the old first-node and default-one assumptions.
...
- The graph records the submitted and canonical URLs separately, plus source metadata, content hash, parser version, candidate count, and selection reason.
