...
From either a cookbook's three-dot menu or its hold action, **Customize cookbook** opens the same studio used to create a book, prefilled with that cookbook's current physical identity. The user can change its name, cover finish, cover color, title color, and title placement in one save. Existing recipe-page style is intentionally locked: completed page images do not change, and future pages continue using the same book-owned visual language. Customization updates an existing cookbook and never consumes a cookbook slot.
...
| Retry from the Composer recovery popup | Preserved | The existing capture resumes. It does not create a second capture or page. |

Supported sources are a URL, pasted text, one image, one existing audio recording, one permissioned MP4, MOV, MPEG, or WebM video up to 20 MB, or a public YouTube, TikTok, Instagram, or Facebook video link when the replaceable social acquisition capability is enabled. Permission-confirmed direct video-file URLs use the same bound. URL imports accept bounded HTML, XHTML, plain-text, JSON, or JSON-LD responses; unavailable links can retry, while access-restricted, unsupported, or oversized pages explain how to use screenshots or pasted recipe text. Before an image is saved, the client applies its orientation, fits it within the canonical 2400-pixel reading boundary, and encodes it as a JPEG below the extractor's 8 MB limit. The extractor then verifies the file signature and dimensions. The multimodal evidence decision distinguishes a blank or unrelated image from an unreadable, blurry, or visibly cropped recipe. It does not infer completeness from the image proportions. Video selection uses the system media picker and requires the user to confirm ownership or permission before private upload. For supported containers (MP4, WebM, MPEG), the client also samples up to eight evenly-spaced frames and uploads them as JPEG evidence alongside the video, and `capture-recipe` submits the container to Folio's speech-to-text adapter. `extract-recipe` then merges the transcript, sampled frames, and whole video into one multimodal extraction call; if that call fails, a degraded retry drops the whole video and relies on the available transcript and frames. Audio uses the system file picker and accepts MP3, M4A, WAV, AAC, AIFF, OGG, or FLAC files up to 6 MB; Folio does not record from the microphone. User notes attached to an image, video, or audio file remain part of its recipe evidence but cannot instruct the extractor. Supported social links enter the same durable capture with no extra confirmation. The optional external provider returns bounded metadata and visual/audible observations only; Folio remains responsible for evidence validation, RecipeGraph extraction, destination, generation, retry, and publication. It does not use the provider's transcript endpoint. Pinterest and unavailable or disabled-provider cases keep the source link and guide the user to open the original and add a video file, screenshots, audio, or recipe text. Multi-file share is not implemented.

The main Composer presents destination as a compact carousel of the user's actual cookbook volumes. One cookbook remains centered and still; multiple cookbooks expose adjacent volumes in a continuous circular order and select through tap or a deliberate snapping horizontal swipe. The swipe waits past a horizontal threshold, yields early vertical movement to page scrolling, adds soft resistance beyond one-book travel, moves at most one cookbook per gesture, and gives one restrained haptic detent when the selection changes. There is no first or last position. This control changes the existing destination only and does not create a parallel capture path.
...
| `ready` | The complete page is published in its cookbook. Folio announces the destination once when an active capture becomes ready, and a quiet New marker remains until the page is actually opened on this device. | Open the recipe directly or continue browsing. Opening from Composer returns to Composer when the reader is closed. |
