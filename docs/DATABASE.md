...
The database transition trigger permits `processing -> needs_destination | ready | needs_attention`, plus `needs_destination -> processing` and `needs_attention -> processing`. Repeating the current state is idempotent. Client roles can read only their own rows. Authenticated creation, destination choice, and settled-capture discard use guarded RPCs; extraction, page creation, failure, and finalization RPCs are restricted to `service_role`. `discard_recipe_capture` accepts only owner-scoped `needs_attention` or `needs_destination` rows. It removes an unpublished processing page and orphan recipe when present, then queues unreferenced source and page objects for Storage cleanup. The client treats a 10-minute-old `processing_started_at` as an abandoned lease. If the worker stopped before claiming the row, it falls back to `updated_at`. Each new `processing_attempt` gets one automatic reclaim, so a second worker failure can recover without another cold launch.
