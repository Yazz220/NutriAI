...
- Folio disables server-side extraction of YouTube, TikTok, and Instagram video links until Folio has written platform authorization or an approved API use that covers this exact workflow.
...
- A photo or video upload is accepted only after the user confirms that they created it or have permission to use it. Folio's terms grant only the license needed for private processing, storage, generation, and deletion.
- Generated cookbook pages stay private to the account. They use new artwork and independently worded functional directions. Folio does not publish, syndicate, sell, or redistribute imported material.

If build 13 still sends social-video URLs for extraction, the truthful Content Rights answer remains **Yes**, but Folio cannot presently substantiate the necessary platform permission. Disable that path before App Review or obtain authorization.
...
Private storage reduces redistribution risk, but it does not cure unauthorized access or copying. A user warranty also does not override a platform's contract with Folio. The importer itself must comply with the source's terms.
...
Launch rule: Folio may retain the URL and offer "Visit original source." It must not fetch the video, audio, captions, transcript, thumbnail, or frames for extraction. An official embed is a separate future feature and must retain YouTube attribution, controls, and policy compliance.
...
- Removing attribution or implying that Folio created the source recipe. Keep a neutral source link where one exists.
