...
| `app/(book)/[cookbookId]/add.tsx` | Source composer for URL, text, image, permissioned video, or audio import |
...
Capture from Share to Nosh, Add page, or assistant handoff
  ...
  -> public YouTube, TikTok, Instagram, and Facebook links may use a replaceable acquisition adapter
     ...
     -> Pinterest and disabled-provider cases retain the guided file, screenshot, audio, or text fallback
  ...
  -> deterministic recipe-quality assessment checks critical field consistency
  -> open blocking issues stop before page creation and use the saved-graph correction surface
  ...
  -> capture-recipe retains source provenance, confidence, and quality diagnostics internally
  -> create_capture_page stores one processing page with clean canonical cooking data
...
1. **Recipe Evidence and Extraction:** Uses deterministic schema.org Recipe JSON-LD or Microdata for structured URLs and a replaceable strict-schema multimodal model for unstructured text, image, video, acquired social-video observations, and audio transcripts. URL acquisition classifies unavailable, access-restricted, unsupported, and oversized responses before extraction, and recognizes common bot-challenge interstitials without treating them as recipe content. Image preflight reads only bounded container headers, trusts the real JPEG, PNG, WebP, or GIF signature over caller metadata, rejects corrupt and implausibly tiny files, and records dimensions and byte size. The multimodal extractor still owns pixel-level judgments about blankness, blur, handwriting, cropping, and recipe completeness. Nosh does not run a second OCR service or raster decoder in the Edge Function. The video adapter accepts permissioned private uploads or permission-confirmed direct files under a 20 MB bound and verifies MP4/MOV, MPEG, or WebM container signatures. Video ingestion uses signal decomposition: `capture-recipe` submits supported containers (MP4, WebM, MPEG) to the same replaceable speech-to-text adapter as standalone audio, while the client samples up to eight evenly-spaced frames and uploads them as supplementary on-screen-text evidence. `extract-recipe` merges the transcript, sampled frames, and the whole video into a single multimodal model call, with a degraded retry that drops the whole video and relies on the available transcript and frames if the first attempt fails. Public YouTube, TikTok, Instagram, and Facebook links may pass through the provider-neutral external-acquisition port in `_shared/recipeEvidenceAcquisition.ts`; the current optional Supadata adapter returns bounded metadata and seen/heard observations, then Nosh's extractor makes the only recipe-evidence decision and constructs the only RecipeGraph. Pinterest retains the guided fallback. External acquisition does not call a transcript endpoint: uploaded audio and video transcription remains inside Nosh's `_shared/audioTranscription.ts` abstraction. Only `recipe` carries a RecipeGraphDraft.
...
`_shared/recipeQuality.ts` runs after either extraction adapter and before destination or page creation. It records its version, routing decision, issue codes, field paths, confirmations, and measured coverage on the internal capture graph. Missing ingredient amounts across an entire usable list, absent oven temperature for a baking method, serving-yield conflicts, invalid or impossible time relationships, and blank cooking fields block because they materially change whether the recipe can be cooked. Inferred details, lower amount coverage, and missing optional duration remain internal warnings and do not interrupt publication or become page notes. `needs_recipe_correction` retains the capture graph and routes the mobile capture detail to `RecipeCorrectionSheet`; a valid submission updates the same capture and re-runs the assessment before generation. A later page retry reuses that accepted quality checkpoint instead of repeating source interpretation.
